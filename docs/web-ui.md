...
- Flow Panel header actions now include `↓ Logs` and `✕ Clear`. `✕ Clear` asks for confirmation, then clears all currently displayed Flow events/turns in the UI (client-side only).
