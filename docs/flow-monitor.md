...
- **Echo suppression for the shared default session**: private DMs and Lumi's own `chat.send` (web chat, sensing, system injections) all land on `sessionKey == agent:main:main`. The `delivery.channel`/`origin.provider` fields are session-cached so they reflect whatever channel touched the session most recently — not always the current message. Two complementary checks suppress Lumi-originated user messages on this shared key:
    - `isLumiInjectedUserMessage()` — prefix match for `[system]`, `[sensing:`, `[ambient]`, `(system)`, heartbeat, wake. Cheap, catches Lumi's structured injections regardless of timing.
