...
- **Filter**: skip up front when `origin.provider == "heartbeat"` (cron heartbeat keeps the lifecycle path so its TTS reaches the lamp speaker). Otherwise treat as a Telegram channel turn when **any** of holds: `sessionKey` starts with `agent:main:telegram:` (primary, stable signal); `origin.provider == "telegram"`; or `deliveryContext.channel == "telegram"`. Lumi's own `chat.send` session (`agentGateway.GetSessionKey()`) is excluded.
...
- **Tool surfacing**: OpenClaw 5.2 gates the `session.tool` broadcast on `isControlUiVisible`, which is `false` for non-webchat channels (verified in `auto-reply/reply/agent-runner-execution.ts` — `shouldSurfaceToControlUi = isInternalMessageChannel(provider)` is true only for `webchat`). Tool surfacing for Telegram therefore comes from the assistant's `content[]` array directly: `extractMessageToolCalls()` pulls `{type:"toolCall", name, arguments}` blocks and emits one `tool_call` flow event per call (phase `end`) under the synthetic run id. This lights up the **Agent Tools** node in the Flow Monitor pipeline. (`case "session.tool"` still maps the UUID → synthetic id defensively in case OpenClaw later loosens the gate.)
