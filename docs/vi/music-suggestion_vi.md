...
Agent match music SKILL trigger → trả lời:
  [HW:/audio/play:{"query":"Take Five Dave Brubeck"}]
...
LeLamp nhận /audio/play:
  → MusicService.play("Take Five Dave Brubeck")
  → yt-dlp search YouTube → ffmpeg stream → ALSA speaker
  → _log_play_event() → ghi vào music_YYYY-MM-DD.jsonl
```
...
| File | Vai trò |
|------|---------|
| `lelamp/service/voice/music_service.py` | yt-dlp search → ffmpeg stream → ALSA. Log play history vào JSONL |
| `lelamp/server.py` | `POST /audio/play`, `POST /audio/stop`, `GET /audio/status`, `GET /audio/history` |
...
### Audio history (`GET /audio/history?date=YYYY-MM-DD&last=N`)

Lưu tại LeLamp `music_YYYY-MM-DD.jsonl`:
...
**Kết quả mong đợi:**
- Agent trả lời với `[HW:/audio/play:{"query":"..."}]` marker
...
# Xem audio history
curl -s "http://<LUMI_IP>:5001/audio/history?last=5"
```
...
```bash
# Qua API
curl -s "http://<LUMI_IP>:5001/audio/history?last=20"
```
