...
- **PRESENT** → gửi `motion.activity` với **activity group** (`drink`, `break`, `sedentary`, `emotional`) thay vì raw action name. Không gửi ảnh — tiết kiệm tokens. **Không** yêu cầu nhận diện friend.
...
3. **Log** từng group quan sát được qua `POST /api/wellbeing/log` với `{action, notes, user}` (mỗi group 1 entry)
4. **Phản hồi caring** dựa trên context từ history (ví dụ: "ly thứ 3 hôm nay rồi á, tốt lắm!"). Quan sát, không chỉ dẫn. KHÔNG BAO GIỜ nhắc cron/timer/reminder.
