...
**Event `llm_first_token` (LLM Start):** Phát ra **đúng 1 lần** mỗi turn ở delta đầu tiên (thinking hoặc assistant) từ stream của OpenClaw. Detail `{ run_id, stream }` với `stream` = `"thinking"` hoặc `"assistant"` (cái nào đến trước). Khoảng `lifecycle_start → llm_first_token` là **LLM warmup / TTFT thực** — trước khi token đầu chảy ra. Khoảng `chat_send → lifecycle_start` là **OpenClaw init turn** (network + load session/context + boot agent), KHÔNG phải LLM. ⚠️ `stream: "thinking"` nghĩa là model **vừa bắt đầu** reasoning (extended thinking), KHÔNG phải đã think xong; với Claude 4.x mặc định extended thinking ON nên `stream` thường là `"thinking"`. Code: `OpenClawHandler.markFirstToken` trong `lumi/server/openclaw/delivery/sse/handler.go`; map `firstTokenSeen` được clear ở `lifecycle_end`/`error`. Lưu ý: turn channel-driven (Telegram queue mode) hiện chưa emit event này vì stream `thinking`/`assistant` của agent path bị OpenClaw 5.x gate.
