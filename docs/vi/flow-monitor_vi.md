...
- **Surface tool calls**: OpenClaw 5.2 gate `session.tool` broadcast bằng `isControlUiVisible` — chỉ true cho channel `webchat` (xác minh ở `auto-reply/reply/agent-runner-execution.ts`: `shouldSurfaceToControlUi = isInternalMessageChannel(provider)`). Telegram → false → không nhận `session.tool`. Do đó tool surfacing cho Telegram lấy trực tiếp từ `content[]` của assistant: `extractMessageToolCalls()` extract block `{type:"toolCall", name, arguments}` và emit 1 `tool_call` flow event/tool (phase `end`) dưới synthetic run id. Nhờ đó node **Agent Tools** ở Flow Monitor pipeline sáng. (`case "session.tool"` vẫn map UUID → synthetic id defensively phòng OpenClaw mở lại gate sau này.)
