...
**Event `llm_first_token`:** Phát ra **đúng 1 lần** mỗi turn ở delta đầu tiên (thinking hoặc assistant) từ stream của OpenClaw. Detail `{ run_id, stream }` với `stream` = `"thinking"` hoặc `"assistant"` (cái nào đến trước). Khoảng `lifecycle_start → llm_first_token` = **LLM warmup / TTFT thực** — trước khi token đầu chảy ra. Khoảng `chat_send → lifecycle_start` = **OpenClaw init turn** (network + load session/context + boot agent), KHÔNG phải LLM. ⚠️ `stream: "thinking"` nghĩa là model **vừa bắt đầu** reasoning (extended thinking / OpenAI Codex thinking=low), KHÔNG phải đã think xong. Code: `OpenClawHandler.markFirstToken` trong `lumi/server/openclaw/delivery/sse/handler.go`; map `firstTokenSeen` clear ở `lifecycle_end`/`error`.
