...
| Vàng `(255,255,0)` | Breathing nhanh | **Hardware Failure** — Lỗi linh kiện | Servo, LED driver, audio, hoặc voice pipeline báo lỗi | Có — khi linh kiện phục hồi |
| Tím `(180,0,255)` | Breathing nhanh | **LeLamp Down** — Server phần cứng không phản hồi | LED tối khi down; tím flash 3s khi phục hồi | Có — tự tắt sau 3s |
...
```
Error (cao nhất) > OTA > Booting > Connectivity > LeLamp Down > Hardware > Agent Down (thấp nhất)
```
...
- Kích hoạt khi servo, LED driver, audio, hoặc voice pipeline báo lỗi qua LeLamp `/health`
