...
func (h *SensingHandler) PostEvent(c *gin.Context) {
	...
	if req.Type == "voice_command" {
		...
		case "motion.activity":
			...
			msg += "\n[MANDATORY: Wellbeing is event-driven (NO cron). On EVERY motion.activity: (1) POST /api/wellbeing/log for each Activity group present (drink/break/sedentary) — backend dedups. (2) GET /api/openclaw/wellbeing-history?user=<current_user>&last=50, compute minutes_since_last_drink, minutes_since_last_break, minutes_since_last_nudge_hydration, minutes_since_last_nudge_break (reset_ts uses max of the activity entry and last enter entry). Apply the Wellbeing skill's Step 4 rules (threshold + nudge cooldown guards) to decide whether to nudge. (3) If you spoke a nudge, POST back with action=nudge_hydration or nudge_break for timeline + cooldown. Never guess time-since — always compute from the log. Music: MUST follow Music skill AI-Driven Music Suggestion workflow for current_user. Emotional cue: follow Emotion Detection skill AND read Mood SKILL.md then execute its full workflow (TWO separate POSTs to /api/mood/log — first kind=signal, then kind=decision; source=camera, trigger=<cue>), ALWAYS speak.]"
		}
	}
